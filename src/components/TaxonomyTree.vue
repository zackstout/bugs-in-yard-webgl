...
const tree = computed<TaxonomyTree>(() => {
  ...
  for (const obs of observations) {
    ...
    if (!orderEntry.familyMap.has(obs.family)) {
      orderEntry.familyMap.set(obs.family, { genusMap: new Map(), unspecified: [] })
    }
    ...
    if (!familyEntry.genusMap.has(obs.genus)) {
      familyEntry.genusMap.set(obs.genus, { speciesMap: new Map(), unspecified: [] })
    }
...
function obsLabel(obs: Observation): string {
  return obs.commonName || obs.scientificName || obs.id
}
...
function groupedLabels(obs: Observation[]): string[] {
  ...
  return Array.from(counts.entries()).map(([label, n]) => n > 1 ? `${label} (${n})` : label)
}
</script>
...
<template>
  <div class="tree-view">
    ...
    <ul class="tree-root">
      ...
      <li v-if="tree.unspecified.length" class="node node-unspecified">
        <span class="label-unspecified">unspecified creature</span>
        <ul>
          <li v-for="label in groupedLabels(tree.unspecified)" :key="label" class="node node-obs">{{ label }}</li>
        </ul>
      </li>

      <!-- Orders -->
      <li v-for="orderNode in tree.orders" :key="orderNode.order" class="node node-order">
        <span class="rank-label">Order</span>
        <span class="taxon-name">{{ orderNode.order }}</span>

        <ul>
          <!-- Observations known only to order level -->
          <li v-if="orderNode.unspecified.length" class="node node-unspecified">
            <span class="label-unspecified">unspecified creature</span>
            <ul>
              <li v-for="label in groupedLabels(orderNode.unspecified)" :key="label" class="node node-obs">{{ label }}</li>
            </ul>
          </li>

          <!-- Families -->
          <li v-for="familyNode in orderNode.families" :key="familyNode.family" class="node node-family">
            <span class="rank-label">Family</span>
            <span class="taxon-name">{{ familyNode.family }}</span>

            <ul>
              <!-- Observations known only to family level -->
              <li v-if="familyNode.unspecified.length" class="node node-unspecified">
                <span class="label-unspecified">unspecified creature</span>
                <ul>
                  <li v-for="label in groupedLabels(familyNode.unspecified)" :key="label" class="node node-obs">{{ label }}</li>
                </ul>
              </li>

              <!-- Genera -->
              <li v-for="genusNode in familyNode.genera" :key="genusNode.genus" class="node node-genus">
                <span class="rank-label">Genus</span>
                <span class="taxon-name">{{ genusNode.genus }}</span>

                <ul>
                  <!-- Observations known only to genus level -->
                  <li v-if="genusNode.unspecified.length" class="node node-unspecified">
                    <span class="label-unspecified">unspecified creature</span>
                    <ul>
                      <li v-for="label in groupedLabels(genusNode.unspecified)" :key="label" class="node node-obs">{{ label }}</li>
                    </ul>
                  </li>

                  <!-- Species -->
                  <li v-for="speciesNode in genusNode.species" :key="speciesNode.scientificName" class="node node-species">
                    <span class="taxon-name taxon-name--species">{{ speciesNode.scientificName }}</span>
                    <span class="obs-summary"> — {{ obsLabel(speciesNode.obs[0]) }}<span v-if="speciesNode.obs.length > 1" class="obs-count"> ({{ speciesNode.obs.length }})</span></span>
                  </li>
                </ul>
              </li>
            </ul>
